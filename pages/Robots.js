...
function Robots({ products }) {
    ...
    return (
        <div >
            <h2 className='text-center text-2xl md:text-3xl'>Best Robots of all time</h2>
                                ...
                                return (
                                    <div key={items._id} className="shadow-lg flex flex-col justify-between w-full h-fit p-2 m-5 md:w-5/12 md:h-96  lg:w-3/12">
                                        <div className="flex justify-center relative rounded overflow-hidden cursor-pointer " onClick={() => goToPage(`product/${items.slug}`)} >
                                            <Image src={`/Products/${items.image}`} width={500} height={500} alt="Product" />
                                        </div>
...
export async function getServerSideProps() {
    ...
    let products = await Product.find({ 'category': 'Robots', 'color': ['White', 'Black'] })
